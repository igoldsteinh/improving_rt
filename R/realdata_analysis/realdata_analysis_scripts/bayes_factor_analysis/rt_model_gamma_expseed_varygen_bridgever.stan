//Variation on state of the art model
//Allow for generation time to change depending on variant which dominates at time
...
data {
  ...
  //weights for variant one
  vector[d] w_one;
  //weights for variant two
  vector[d] w_two;
  //weights for variant three
  vector[d] w_three;
...
transformed parameters{
  ...
  { int c;
    ...
    for (t in 1:n) {
      ...
      if (t >= change_two && t < change_three){
          if (c <= d ) {
        
        weighted_sum[t] = dot_product(head(aug_incid, c), 
                                       my_reverse(head(w_two, c)));
