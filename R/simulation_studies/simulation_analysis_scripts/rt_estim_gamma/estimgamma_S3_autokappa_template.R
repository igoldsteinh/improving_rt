...
# using S3 data set choosing kappa automatically as detailed in methods section
...
spline_fit <- run_nb_spline(weekly_sim_data)

kappa_params <- choose_kappa_params(spline_fit)
# fit model ---------------------------------------------------------------
...
model_objects_gamma_seed_bb6 <- list(n = data_length, 
                                     ...
                                     log_r0_mu = log(1),
                                     log_r0_sd = .75,
                                     kappa_mu = kappa_params$par[1],
                                     kappa_sd = kappa_params$par[2])
...
write_rds(res, str_c("S3_autokappa_gamma_res", seed, ".rds", ""))
